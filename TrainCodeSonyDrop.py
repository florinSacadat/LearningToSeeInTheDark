...
my_saved_model = '/media/student/SPATIU/LTSID/Last model/MODEL_D'
...
for epoch in range(lastepoch, 46):
    ...
    for batch_idx, (inputs, targets) in enumerate(train_loader):
        ...
        if epoch % save_freq == 0 :
            if not os.path.isdir(result_dir + '%04d' % epoch+'D'):
                os.makedirs(result_dir + '%04d' % epoch+'D')
            ...
            ToImage.toimage(temp * 255, high=255, low=0, cmin=0, cmax=255).save(
                result_dir + '%04dD/_00_train_%d.jpg' % (epoch, batch_idx))
            torch.save(model.state_dict(), model_dir + 'checkpoint_sony_e%04d' % epoch+'D')
    ...
    torch.save(model.state_dict(),
               '/media/student/SPATIU/LTSID/Last model/MODEL_D')
